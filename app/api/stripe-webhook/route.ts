...
function sha256(value?: string | null) {
  return value ? crypto.createHash('sha256').update(value.trim().toLowerCase()).digest('hex') : undefined;
}
...
async function sendMetaPurchase(session: Stripe.Checkout.Session) {
  ...
  const payload = { data: [{
    ...
    user_data: { em: email ? [sha256(email)] : undefined, ph: phone ? [sha256(phone)] : undefined },
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (supabase) await supabase.from('stripe_events').delete().eq('event_id',event.id);
