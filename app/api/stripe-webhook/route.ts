...
async function sendMetaPurchase(session: Stripe.Checkout.Session) {
  ...
  if (!response.ok) {
    console.error('meta_capi_error', response.status, await response.text());
  }
}
...
async function persistSession(session: Stripe.Checkout.Session) {
  ...
  const { error } = await supabase.from('orders').upsert(
    {
      ...
      fulfilment_status: paid ? 'paid' : 'awaiting_payment',
  ...
  if (error) console.error('order_upsert_error', error);
}
