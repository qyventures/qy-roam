...
async function sendMetaPurchase(session: Stripe.Checkout.Session, eventTime: number) {
  ...
  // This request contains exactly one Purchase. A 2xx only proves that the
...
async function sendHumanFulfilmentEmail(session: Stripe.Checkout.Session) {
  ...
  if(configuredRelayUrl&&relaySecret){
    ...
    // configuration has changed. Revalidate beside the credential-bearing
    // request so SMTP credentials and paid-order PII are never sent over HTTP
