...
export async function POST(req: Request) {
 try {
  ...
  const body=await req.json(); const country=String(body.country||''); const daily=DAILY_RATES[country]; const startDate=parseDate(body.start); const endDate=parseDate(body.end);
  ...
  const stripe=new Stripe(key,{apiVersion:'2024-06-20'}); const origin=process.env.NEXT_PUBLIC_SITE_URL||new URL(req.url).origin;
