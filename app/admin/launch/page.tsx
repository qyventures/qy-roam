...
export default async function LaunchPage(){
 const dbOk=await hasRequiredPaymentSchema();
 ...
 <div style={{...card,marginTop:16}}><h2 style={{marginTop:0}}>Launch checklist</h2><Row label="Stripe checkout" ok={stripe} note="Server-side Stripe credential"/><Row label="Stripe webhook" ok={webhook} note="Required for signed payment confirmation and durable order processing"/><Row label="Payment database schema" ok={dbOk} note="Orders plus Stripe idempotency, fulfilment and Meta delivery ledgers"/><Row label="Fulfilment email" ok={smtp} note="Paid-order alert to QY operations for manual fulfilment"/><Row label="Meta Pixel" ok={pixel} note="Consent-gated browser measurement"/><Row label="Meta CAPI" ok={capi} note="Server-side Purchase measurement"/><Row label="Production URL" ok={site} note="qyroam.com"/></div>
