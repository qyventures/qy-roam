...
export default async function LaunchPage(){
 ...
 // used by the webhook is available. A non-empty placeholder Pixel ID or an
 ...
 const commonCheckoutReady=stripe&&webhook&&site&&orderIntegrity&&smtp;
 const wifiInventory=Boolean(config&&config.pocketWifiInventory>0);
 const esimReady=commonCheckoutReady&&esimOrderDbOk;
 const wifiReady=commonCheckoutReady&&paymentDbOk&&pocketWifiFulfilmentDbOk&&wifiInventory;
 const paidReady=esimReady&&wifiReady&&metaCapi;
 const commonBlockers=[!stripe&&'live Stripe credential',!webhook&&'Stripe webhook signing secret',!site&&'production HTTPS site URL',!orderIntegrity&&'order-integrity signing secret',!smtp&&'fulfilment email transport'].filter(Boolean) as string[];
 const esimBlockers=[...commonBlockers,!esimOrderDbOk&&'eSIM order database schema'].filter(Boolean) as string[];
 const wifiBlockers=[...commonBlockers,!paymentDbOk&&'Pocket WiFi payment and reservation schema',!pocketWifiFulfilmentDbOk&&'Pocket WiFi dispatch and return custody schema',!wifiInventory&&'Pocket WiFi saleable-capacity setting'].filter(Boolean) as string[];
 const paidBlockers=Array.from(new Set([...esimBlockers,...wifiBlockers,!metaCapi&&'valid Meta Pixel ID and CAPI access token'].filter(Boolean))) as string[];
