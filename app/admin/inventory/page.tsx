...
async function loadReservationPages(db:NonNullable<ReturnType<typeof getSupabaseAdmin>>):Promise<ReservationPageResult>{
 const data:any[]=[];
 // Checkout retains an expired reservation for four days while Stripe retries
 // a terminal webhook. Show the same capacity set used by the database RPC,
 // not just rows whose customer-facing Checkout window is still open.
 ...
 for(let from=0;from<RESERVATION_MAX_ROWS;from+=RESERVATION_PAGE_SIZE){
  const result=await db.from('checkout_reservations').select('checkout_request_id,stripe_session_id,travel_start,travel_end,expires_at,created_at').gt('expires_at',cutoff).order('expires_at').order('checkout_request_id').range(from,from+RESERVATION_PAGE_SIZE-1);
  if(result.error) return {data:[],error:result.error,truncated:false};
  const page=result.data||[];
  data.push(...page);
  if(page.length<RESERVATION_PAGE_SIZE) return {data,error:null,truncated:false};
 }
 const beyond=await db.from('checkout_reservations').select('checkout_request_id').gt('expires_at',cutoff).order('expires_at').order('checkout_request_id').range(RESERVATION_MAX_ROWS,RESERVATION_MAX_ROWS);
 if(beyond.error) return {data:[],error:beyond.error,truncated:false};
 return {data,error:null,truncated:(beyond.data||[]).length>0};
}
