...
export default async function AdminPage() {
  ...
  const dispatchExceptions = active.filter((o:any) => {
  ...
  return <main className="wrap section legal">
    ...
    {supabase && <>
      ...
      {(dispatchExceptions.length > 0 || returnExceptions.length > 0) && <p><strong>Attention:</strong> {dispatchExceptions.length} dispatch exception{dispatchExceptions.length === 1 ? '' : 's'} · {returnExceptions.length} overdue return{returnExceptions.length === 1 ? '' : 's'}</p>}
    </>}
    ...
    {orders.length > 0 && <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}>
      ...
      <tbody>{orders.map((o:any)=>{
        ...
        return <tr key={o.id} style={{borderTop:'1px solid #e5e8ed',verticalAlign:'top'}}>
          ...
          <td style={{padding:'14px 8px'}}>{o.country}<br/><small>{o.travel_start} → {o.travel_end}</small>{flag && <><br/><small><strong>{flag}</strong></small></>}</td>
