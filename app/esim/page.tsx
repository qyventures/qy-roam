...
export default function EsimPage() {
  ...
  return <main>
    <section className="promo-banner"><div className="wrap promo-inner"><div><span className="promo-kicker">{ESIM_PROMO.label}</span><strong>15% below verified comparable public benchmark pricing</strong><span>Launch pricing is applied automatically at checkout</span></div><a href="#esim-plans" className="promo-cta">Shop eSIM</a></div></section>

    <section className="hero esim-hero"><div className="wrap hero-grid"><div><span className="eyebrow">Travel eSIM</span><h1>Land connected. No SIM swapping required.</h1><p className="lead">Choose your destination, buy online and install your eSIM by QR code. QY Roam launch prices are set server-side at 15% below the verified comparable public benchmark for each listed plan.</p><div className="trust-row"><span>QR-code setup</span><span>No physical SIM swap</span><span>Hotspot on supported plans</span></div></div><div className="search-card"><span className="promo-kicker esim-kicker">LAUNCH PRICE</span><h2>Choose your eSIM</h2><label>Plan<select value={planId} onChange={(e) => setPlanId(e.target.value)}>{ESIM_PLANS.map((item) => <option value={item.id} key={item.id}>{item.destination} · {item.days} days</option>)}</select></label><div className="promo-applied">✓ 15% benchmark discount already included · no code needed</div><button className="primary" onClick={checkout} disabled={busy}>{busy ? 'Opening secure checkout…' : `Buy for S$${plan.qyPriceSgd.toFixed(2)}`}</button>{error && <p className="muted">{error}</p>}<small>Card & PayNow where available. eSIM-compatible, carrier-unlocked device required.</small></div></div></section>

    <section className="wrap section" id="esim-plans"><div className="section-head"><div><span className="eyebrow">Travel eSIM plans</span><h2>Launch selection</h2></div><p>We list only plans with a verified comparable public benchmark and expand the catalogue as additional destination pricing is validated.</p></div><div className="esim-grid">{ESIM_PLANS.map((item) => <article className="esim-card" key={item.id}><span className="pill">eSIM</span><h3>{item.destination}</h3><p>{item.days} days · {item.data}</p><p className="muted">{item.note}</p><div className="esim-price-row"><div><span className="old-total">Benchmark S${item.benchmarkPriceSgd.toFixed(2)}</span><strong>S${item.qyPriceSgd.toFixed(2)}</strong><small> QY Roam launch price</small></div><button className="secondary" onClick={() => { setPlanId(item.id); document.querySelector('.esim-hero')?.scrollIntoView({ behavior: 'smooth' }); }}>Choose</button></div></article>)}</div></section>
