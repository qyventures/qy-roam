...
const statuses = ['paid','packing','dispatched','with_customer','return_due','returned','closed','cancelled'];

export default function AdminOrderActions({ id, initialStatus, courierTracking = '', returnTracking = '' }: { id: number; initialStatus: string; courierTracking?: string | null; returnTracking?: string | null }) {
  ...
  async function save() {
    ...
    try {
      const res = await fetch(`/api/admin/orders/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status, courier_tracking: courier, return_tracking: returned }) });
